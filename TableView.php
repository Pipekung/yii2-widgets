...
class TableView extends Widget {
	...
	public function init() {
		...
		if (isset($this->action)) {
			...
			$this->action = [
				'template' => [
					'view' => '<a class="btn btn-minier btn-default" href=""><i class="glyphicon glyphicon-th-list"></i></a>',
					'update' => '<a class="btn btn-minier btn-primary" href=""><i class="glyphicon glyphicon-pencil"></i></a>',
					'delete' => '<button type="button" class="btn btn-minier btn-danger"><i class="glyphicon glyphicon-trash"></i></button>',
				],
	...
	public function run() {
		...
		foreach ($this->data as $val) {
			...
			if ($this->action['template']['update']['inline']) {
				$TbData .= $this->renderDataWithUpdate($val);
			} else {
				$TbData .= $this->renderData($val);
			}
	...
	public function renderButton($val) {
		if (in_array('view', $this->action['template']) || isset($this->action['template']['view'])) {
			if (is_array($this->action['template']['view'])) {
				return '<a class="btn btn-minier btn-default" href="{{url-to-view}}"><i class="glyphicon glyphicon-th-list"></i></a>';
			} else {
				return '<a class="btn btn-minier btn-default" href="{{url-to-view}}"><i class="glyphicon glyphicon-th-list"></i></a>';
			}
		...
		if (in_array('update', $this->action['template']) || isset($this->action['template']['update'])) {
			if (is_array($this->action['template']['update']) && !$this->action['template']['update']['inline']) {
				return '<a class="btn btn-minier btn-primary" href="' . Url::to([$this->action['template']['update']['url'], 'id' => $val['id']]) . '"><i class="glyphicon glyphicon-pencil"></i></a>';
			} elseif ($this->action['template']['update']['inline']) {
				return $this->renderEditInline($val);
			} else {
				return '<a class="btn btn-minier btn-primary" href="' . Url::to(['update', 'id' => $val['id']]) . '"><i class="glyphicon glyphicon-pencil"></i></a>';
			}
		...
		if (in_array('delete', $this->action['template']) || isset($this->action['template']['delete'])) {
			if (is_array($this->action['template']['delete'])) {
				return '<button type="button" class="btn btn-minier btn-danger"><i class="glyphicon glyphicon-trash"></i></button>';
			} else {
				return '<button type="button" class="btn btn-minier btn-danger"><i class="glyphicon glyphicon-trash"></i></button>';
			}
	...
	public function renderEditInline($val) {
		return '<a class="btn btn-minier btn-primary pk-update-inline" id="' . $val['id'] . '" href="#' . $val['id'] . '"><i class="glyphicon glyphicon-pencil"></i></a>
        <a class="btn btn-minier btn-success pk-confirm-update hide" id="' . $val['id'] . '" href="#' . $val['id'] . '"><i class="glyphicon glyphicon-ok"></i></a>
        ';
	}
